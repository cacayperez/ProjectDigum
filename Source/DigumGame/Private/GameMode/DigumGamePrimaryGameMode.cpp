...
void ADigumGamePrimaryGameMode::PostInitializeComponents()
{
	Super::PostInitializeComponents();
		...
		if(Asset)
		{
			...
			ProceduralActor->GenerateMap(TEXT("Hello World"), GridSize,24, 24, 256, 256, 2, Asset);
...
void ADigumGamePrimaryGameMode::StartPlay()
{
	...
	// bInPlayMode = true;
	ProceduralActor->SpawnChunks(FVector::ZeroVector, 2);
}
	...
	const FDigumWorldProceduralSectionCoordinate& InPreviusCoordinate)
{
	if(Actor)
	{
		ProceduralActor->SpawnChunks(InCurrentCoordinate, 2);
	}
}
